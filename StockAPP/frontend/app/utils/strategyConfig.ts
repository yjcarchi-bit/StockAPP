export type StrategyType = 'etf_rotation' | 'large_cap_low_drawdown' | 'dual_ma' | 'rsi' | 'macd' | 'bollinger' | 'grid' | 'fms' | 'steal_dog' | 'multi_etf_rotation' | 'multi_strategy_portfolio';
...
const compoundStrategies: Strategy[] = [
  ...
  },
  {
    ...
    type: '多因子选股',
    color: '#e377c2',
    description: '基于ROE和小市值的选股策略。筛选盈利能力强（ROE>15%）的公司，选择市值最小的股票进行持有。该策略结合基本面和技术面指标，通过周度调仓实现收益优化。',
    logic: [
    ...
    适用场景: '适合价值投资风格，追求长期稳健收益的投资者',
    风险提示: '小市值股票波动较大，可能面临较大的回撤风险',
  },
  {
    id: 'steal_dog',
    name: '偷鸡摸狗策略',
    ...
    type: '动量策略',
    color: '#bcbd22',
    description: '基于动量因子的ETF轮动策略（精选版）。在黄金、纳指、创业板三只ETF之间进行动量轮动，持有动量最强的ETF。采用加权线性回归计算动量得分，结合止损机制控制风险。',
    logic: [
    ...
    适用场景: '适合追求稳健收益、希望参与多市场配置的投资者',
    风险提示: '动量反转时可能产生较大回撤，三只ETF均为高风险品种',
  },
  {
    id: 'multi_etf_rotation',
    name: '多ETF轮动策略',
    icon: '🌐',
    category: 'compound',
    type: '动量策略',
    color: '#17becf',
    description: '基于动量因子的多ETF轮动策略（扩展版）。在11只精选ETF之间进行动量轮动，覆盖海外、商品、宽基、债券、行业等多个类别。采用加权线性回归计算动量得分，选择动量最强的ETF持有。',
    logic: [
    ...
    适用场景: '适合追求多元化资产配置、希望参与全球市场投资的投资者',
    风险提示: '多市场轮动可能增加交易成本，动量反转时可能产生较大回撤',
  },
  {
    id: 'multi_strategy_portfolio',
    name: '多策略组合',
    icon: '📦',
    category: 'compound',
    type: '组合策略',
    color: '#8c564b',
    description: '将多个子策略按比例组合的投资策略。每个子策略独立管理自己的子组合，通过资金分配和策略分散实现风险分散和收益优化。默认配置：搅屎棍策略43%、偷鸡摸狗策略22%、多ETF轮动策略35%。',
    logic: [
    ...
    适用场景: '适合追求稳健收益、希望分散投资风险的投资者',
    风险提示: '多策略组合可能降低单一策略风险，但仍受市场系统性风险影响',
  },
];
