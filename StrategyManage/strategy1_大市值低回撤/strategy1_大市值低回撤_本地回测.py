...
本地回测版本说明：
- 使用 efinance 库从东方财富获取历史数据
- 本地模拟回测，不涉及真实交易
- 实现完整策略逻辑：六因子选股 + 回撤锁定 + 动态仓位管理
...
运行方式：
    /usr/bin/python3 strategy1_大市值低回撤_本地回测.py
"""
...
class LocalBacktestStrategy:
    ...
    def __init__(self, start_date: str = None, end_date: str = None, 
        ...
        参数:
            start_date: 回测开始日期，格式 'YYYY-MM-DD'，默认为一个月前
        ...
        if start_date is None:
            start_dt = datetime.now() - timedelta(days=45)  # 预留缓冲
    ...
    def get_index_data(self) -> pd.DataFrame:
        ...
        try:
            # 多获取一些数据用于计算均线
            start_dt = datetime.strptime(self.start_date, '%Y-%m-%d') - timedelta(days=120)
    ...
    def check_trend_recovery(self, date: str) -> Tuple[bool, str]:
        """
        检查趋势是否恢复（解锁条件判断）
    ...
    def select_stocks(self, date: str) -> List[str]:
        ...
        if self.config.drawdown_lock:
            is_unlock, unlock_reason = self.check_trend_recovery(date)
    ...
    def plot_chart(self, save_path: str = 'backtest_chart_strategy1.png'):
        """
        生成回测结果图表
        ...
        self._plot_nav_curve(ax1, df)
        self._plot_drawdown(ax2, df)
        self._plot_position_count(ax3, df)
        self._plot_cash_vs_position(ax4, df)
        self._plot_daily_return_dist(ax5, df)
    ...
    def _plot_nav_curve(self, ax, df):
        """绑制净值曲线"""
        dates = df['date']
        nav = df['total_value']
    ...
    def _plot_position_count(self, ax, df):
        """绑制持仓数量变化"""
        dates = df['date']
        pos_count = df['position_count']
    ...
    def _plot_cash_vs_position(self, ax, df):
        """绑制现金与持仓市值对比"""
        dates = df['date']
        cash = df['cash']
    ...
    def _plot_daily_return_dist(self, ax, df):
        """绑制日收益率分布"""
        df['daily_return'] = df['total_value'].pct_change()
    ...
    def save_results(self):
        ...
        if len(self.daily_values) > 0:
            df = pd.DataFrame(self.daily_values)
            df.to_csv('backtest_daily_values_strategy1.csv', index=False, encoding='utf-8-sig')
            print(f"\n每日净值已保存至: backtest_daily_values_strategy1.csv")
        
        if len(self.trade_records) > 0:
            df = pd.DataFrame(self.trade_records)
            df.to_csv('backtest_trade_records_strategy1.csv', index=False, encoding='utf-8-sig')
            print(f"交易记录已保存至: backtest_trade_records_strategy1.csv")
